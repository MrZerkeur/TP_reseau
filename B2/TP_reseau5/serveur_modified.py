    ...
    try:
        ...
        while True:
            # Receive data from the client
                ...
                message_without_space = message.replace(" ","")
                operators = "+-*/"
                if accept == True:
                    for operator in operators:
                        if operator in message_without_space:
                            # Use split method to split the string based on the operator
                            parts = message_without_space.split(operator, 1)
